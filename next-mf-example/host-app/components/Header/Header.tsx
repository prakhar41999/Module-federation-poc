...
const Header = () => {
  ...
  return (
    <div className={styles.headerWrapper}>
      <div className={styles.gamesEditionWrapper}>
        Header from host app

      </div>
      <div className={styles.profileWrapper}>
        <div className={styles.profileIconWrapper}>
          <AccountDropdown
            avatarText={userSession?.user?.image || 'p'}
            avatarType="initials"
            email={userSession?.user?.email || 'p'}
            isIconOnly
            isLeadingIconForListOptions
            name={userSession?.user?.name || 'p'}
