...
export const authOptions: NextAuthOptions = {
  pages: {
    // Must be paths only (no hostname). Next-auth prepends NEXTAUTH_URL automatically.
    signOut: '/signout',
    signIn: '/',
  },
  ...
  callbacks: {
    async redirect({ url, baseUrl }) {
      ...
      // Allow same-origin and relative paths
      if (url.startsWith(baseUrl) || url.startsWith('/')) {
        return url;
      }

      // Allow explicitly trusted remote app origins (e.g. dtec-app1 on localhost:3001)
